// api/chat.js - CommonJS 版本，避免 ES module 警告
module.exports = async (req, res) => {
  ...
  async function fetchBochaSearch(query) {
    ...
    if (!bochaKey) return "错误：未配置 BOCHA_API_KEY";
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return `HTTP ${response.status}: ${errorText.slice(0, 200)}`;
      }
      const data = await response.json();
      let results = data.results || data.items || data.data || [];
      if (!results.length) return "搜索结果为空";
      let context = "【联网搜索结果】\n";
  ...
  try {
    ...
    if (role === 'master') {
      ...
      if (needSearch) {
        ...
        if (searchResult.startsWith("HTTP") || searchResult.startsWith("错误") || searchResult.startsWith("请求异常") || searchResult === "搜索结果为空") {
