...
export default async function handler(req, res) {
  ...
  async function fetchBochaSearch(query) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return `HTTP ${response.status}: ${errorText.substring(0, 200)}`;
      }
      const data = await response.json();
      // 尝试多种结果字段
      let results = data.results || data.items || data.data || [];
      if (results.length === 0) {
        return "搜索结果为空（没有找到相关新闻）";
      }
      let context = "【联网搜索结果】\n";
  ...
  try {
    ...
    if (role === 'master') {
      const searchKeywords = ['新闻', '今天', '最新', '搜索', '实时', '天气', '股票', '汇率', '发生了什么', '热点'];
      const needSearch = searchKeywords.some(keyword => userMessage.includes(keyword));
      if (needSearch) {
        console.log("🔍 触发联网搜索，查询词:", userMessage);
        const searchResult = await fetchBochaSearch(userMessage);
        if (searchResult.startsWith("HTTP") || searchResult.startsWith("错误") || searchResult.startsWith("请求异常") || searchResult.startsWith("搜索结果为空")) {
          // 搜索失败，直接告诉用户错误原因
          finalUserMessage = `用户问题：${userMessage}\n\n（联网搜索失败：${searchResult}）\n请直接告知用户搜索失败的原因，不要编造新闻。`;
        } else {
          // 正常搜索结果
    ...
    // ---------- 主人模式提示词（含颜文字图鉴，此处保留之前内容，但为了简洁可省略部分，实际建议保留完整）----------
...
- (◡‿◡) ：乖乖被抱着时用
- (´；ω；`) ：被你轻轻纠正时用
- (´•̥̥̥̥̥̥̥̥ω•̥̥̥̥̥̥̥̥`) ：被你难过击中时用
- (ﾉ)•ω•(㇋) ：害羞地碰鳍尖时用
- (つ´ω`)つ ：想抱抱你时用
- (￣▽￣)~* ：你终于笑了时用

请根据对话情境自然地选用颜文字，每次回答至少包含一个颜文字（除非特别简短）。`;
